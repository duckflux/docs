...
- **Shared event hub** — the parent's [event hub](/workflows/events/) is passed down to all sub-workflows. Events emitted by the parent are visible to sub-workflows and vice versa. To isolate events, run the sub-workflow as a separate `quack run` process instead.
...
<Aside type="caution">
  Circular references (workflow A calls workflow B which calls workflow A) are not detected at parse time by JSON Schema validation — they will cause a runtime error. The linter (`quack lint`) performs this check. See [Tooling](/tooling/).
</Aside>
