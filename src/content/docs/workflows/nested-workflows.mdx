...
A participant of type `workflow` delegates execution to another `.duck.yaml` file:

```yaml
participants:
  reviewCycle:
    type: workflow
    path: ./review-loop.duck.yaml
    input:
...
flow:
  ...
  - if:
      ...
      then:
        - as: deploy
          type: workflow
          path: ./deploy.duck.yaml
          input:
            env: workflow.inputs.targetEnv
      else:
        - as: notify
          type: workflow
          path: ./notify-failure.duck.yaml
```
...
flow:
  - parallel:
      ...
      - as: securityScan
        type: workflow
        path: ./security.duck.yaml
      - as: buildArtifact
        type: workflow
        path: ./build.duck.yaml
```
