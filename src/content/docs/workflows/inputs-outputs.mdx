...
| `description` | Human-readable description (used in `quack validate` output) |
...
# JSON file
quack run deploy.duck.yaml --input-file inputs.json

# Piped JSON via stdin
echo '{"branch": "main", "env": "staging"}' | quack run deploy.duck.yaml
```
