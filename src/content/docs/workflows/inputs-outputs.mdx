...
```bash
# Inline flags
duckflux run deploy.duck.yaml --input branch=main --input env=staging

# JSON file
duckflux run deploy.duck.yaml --input-file inputs.json

# Piped JSON via stdin
echo '{"branch": "main", "env": "staging"}' | duckflux run deploy.duck.yaml
```
...
When invoking a reusable participant in the flow, you can override its `input` mapping for that specific call. The flow override **merges** with the participant's declared `input` instead of replacing it:
