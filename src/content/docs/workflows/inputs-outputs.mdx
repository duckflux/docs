...
<Aside type="tip">
  Use participant-level `input` as the default mapping, and flow-level overrides when a specific context requires different data — for example, feeding reviewer feedback back into the coder on a second pass.
</Aside>
