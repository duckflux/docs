...
<Aside>
  Events emitted via `emit` are also propagated **internally** within the same workflow. A `wait` step can react to events published by earlier steps in the same execution. See [Variables and Expressions](/workflows/variables-and-expressions/) for the `event` variable available inside `wait` blocks.
</Aside>
