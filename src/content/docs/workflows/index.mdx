...
|-------|----------|------|-------------|
| [`version`](#version) | no | string | Spec version. Default: `"0.3"`. |
| [`id`](#id-and-name) | no | string | Unique identifier for the workflow definition. |
| [`name`](#id-and-name) | no | string | Human-readable name. |
| [`defaults`](#defaults) | no | object | Global defaults applied to all participants. |
| [`inputs`](#inputs) | no | object | Parameters the workflow accepts from its caller. |
| [`participants`](#participants) | no | object | Named, reusable step definitions. |
| [`flow`](#flow) | **yes** | array | The execution sequence. |
| [`output`](#output) | no | string or object | Final result mapping. Defaults to the last step's output. |
...
1. Flow-level override
2. Participant-level
3. `defaults.timeout`
4. Runtime default (no timeout)

**Working directory precedence** (highest to lowest):
...
See [Working directory](/runtime/working-directory/) for details.
...
Examples: `deploy.duck.yaml`, `review-loop.duck.yaml`, `ci.duck.yaml`.

The `.duck.yaml` double extension makes workflows easy to distinguish from regular config files and glob with a single pattern (`**/*.duck.yaml`).
