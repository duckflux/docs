...
|---|---|---|
| `.github/workflows/*.yml` | `.flux.yaml` file | Both are declarative YAML. |
...
<Tabs>
  ...
  <TabItem label="duckflux">
    ```yaml title="ci-pipeline.flux.yaml"
    inputs:
      ref:
        type: string
        default: "main"
...
<Tabs>
  ...
  <TabItem label="duckflux">
    ```yaml title="matrix-test.flux.yaml"
    flow:
      - parallel:
          - as: test-node-18
            type: exec
...
jobs:
  run:
    ...
    steps:
      ...
      - name: Run pipeline
        run: duckflux run ci-pipeline.flux.yaml
        env:
...
The `.flux.yaml` file is portable. You can run the exact same file locally:

```bash
export DEPLOY_TOKEN=my-local-token
duckflux run ci-pipeline.flux.yaml
```
...
<Aside type="tip">
This hybrid approach lets you migrate incrementally. Move one job at a time from GHA YAML to a `.flux.yaml` file. Keep the GHA workflow as a thin wrapper that triggers `duckflux run`.
</Aside>
...
5. **Write the `.flux.yaml` flow.** Use `parallel:` for independent steps, `when` for conditionals, `loop` for retries.
