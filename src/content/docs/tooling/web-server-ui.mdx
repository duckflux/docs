...
The **quack server** is a local web UI for running and inspecting workflows — think GitHub Actions, but for your duckflux flows. It ships as the `@duckflux/server` package and is launched via the [`quack server`](/runtime/cli/#server) CLI command.
...
<Aside type="note">
  The `--trace-dir` flag is consistent with the [`quack run --trace-dir`](/runtime/cli/#flags) flag. Point both commands at the same directory to see CLI-triggered executions in the server UI.
</Aside>
